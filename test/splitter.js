...
contract('Splitter', (accounts) => {
  ...
  it("owner can deposit to contract", async () => {
    ...
    assert.equal(depositTxObject.logs[0].args.depositAmount, depositAmount, "Deposit amount  is incorrect");
    ...
    assert.strictEqual((contractStartingBalance.plus(depositAmount)).toString(10), 
    (await web3.eth.getBalance(split.address)).toString(10), "Contract balance is incorrect");
    assert.strictEqual((ownerStartingBalance.minus(depositAmount).minus(transCost)).toString(10), 
  ...
  it("receivers can withdraw from contract", async () => {
    ...
    const depositTxObject = await split.deposit(receiver1, receiver2, { from: owner, value: depositAmount });
    ...
    //Checking the balances on the blockchain
    assert.strictEqual(receiver1StartingBalance.plus(depositAmount / 2).minus(transCostReceiver1).toString(10), 
    (await web3.eth.getBalance(receiver1)).toString(10), "Balance of receiver1 is incorrect");
    assert.strictEqual(receiver2StartingBalance.plus(depositAmount / 2).minus(transCostReceiver2).toString(10), 
    (await web3.eth.getBalance(receiver2)).toString(10), "Balance of receiver1 is incorrect");
    assert.strictEqual(contractStartingBalance.minus(depositAmount).toString(10), 
    (await web3.eth.getBalance(split.address)).toString(10), "Contract balance is incorrect");
  })
})
